            ...
            <form action="{{route('admin_home_put', ['id' => $home->id])}}" method="post" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control" id="category" name="category">
                        @foreach($categories as $category)
                            <option value="{{$category->id}}"
                                    @if($home->home_category_id === $category->id)
                                    selected
                                @endif>
                    ...
                    <textarea class="textarea" placeholder="Place some text here" id="content" name="contents"
                              style="width: 100%; height: 500px; font-size: 14px; line-height: 18px; border: 1px solid #dddddd; padding: 10px;">{{$home->content}}</textarea>
